...
Descripci� del codi amb el qual implementem el m�tode Naive Bayes.
...
Descripci� pas a pas de les variables i funcions implementades.

Primer de tot el que fem �s declarar dues variables que ens servir�n per guardar tan la suma de probabilitats de cada paraula de que sigui spam o ham.
...
Aqu� el que fem �s declarar dos enters que ens guarden el total de paraules que tenim registrades en la base de dades en mem�ria com a spam i com a ham.
...
Tan la suma de probabilitats de cada paraula del missatge com la probabilitat del missatge de ser spam o ham, est�n logarimitzades, i per tant el que fem al final �s sumar aquestes dades.
...
Finalment apliquem la f�rmula amb phi quen ens permet donar import�ncia al fet de  no classificar missatges Ham com a Spam, per tant com que volem estar segurs d'aix� fem el seg�ent:
mirem que per classificar un missatge com a Spam, la probabilitat de ser spam ha de ser m�s gran que phi vegades la probabilitat de que sigui ham.
...
// TODO : Com utilitzar el jar i exemples d'execució. 
...
Analitzant el què representen els valors de phi, el que ens trobem és que la phi és el factor d'increment de la probabilitat que un correu sigui considerat SPAM. És a dir un valor de phi = 2, provoca que un correu només serà considerat spam si la seva probabilitat és 2 cops superior a la probabilitat de ser ham. Un valor de phi = 1 fa que no hi hagi increment obligatori per a la comparació.
...
## Qu� passa quan lemmatitzem.

Com hem citat abans, l'�s de la llibreria Stanford Core NLP, ens permet distingir tipus de paraules, trobar l'arrel de les mateixes, etc. N'hem volgut fer un an�lisi tan temporal com en efic�cia, i aquestes s�n les conclusion a les que hem pogut arribar:

Si ens basem en el temps, hem vist que el fet d'utilitzar el filtre amb la llibreria Stanford Core NLP, augmenta consedrablement, parlem de l'ordre de 100 vegades m�s lent que si no utilitzem un filtratge de paraules.


Tamb� podem veure que parlant d'efic�cia, l'utilitzaci� de la llibreria Stanford Core NLP, ens d�na resultats amb valors d'accuracy inferiors al 90%. Podem veure un exemple d'execuci� amb 22.613 ,missatges:
...
Comparant amb un exemple d'execuci� sense filtre, que ens sol donar valors d'accuracy entre un 97% i 99%, un exemple d'execuci� seria:
